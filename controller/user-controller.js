...
const registerUsers = (req, res) => {
    const { userID, userName, emailID, password, profilePic, created_at, updated_at } = req.body;

    // Insert user data into the MySQL database
    const registerQuery = 'INSERT INTO user_table (userID, userName, emailID, password, profilePic, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())';
    connection.query(registerQuery, [userID, userName, emailID, password, profilePic, created_at, updated_at], (error, results) => {
      if (error) {
        console.error('Error registering user in MySQL:', error);
        //Internal Server Error
        res.status(500).json({ error: '500 : Registration failed' });
      } else {
        res.status(201).json({ message: '200 : User registered successfully' });
      }
    ...
    if (error) {
      ...
      if (results.length === 0) {
        ...
        if (password === user.password) {
          ...
          res.status(200).json({ message: '200 : Login successful', user });
        } else {
          // Passwords do not match : Authentication failed
          res.status(401).json({ error: '401 : Passwords do not match' });
        }
      }
    }
  });
